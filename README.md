...
在 [Releases](../../releases) 选择实际已上传的安装包。Windows 0.2.2 是同一软件的原位更新，当前用户安装，无需另装 Python。Windows 安装器检查微软 WebView2 运行时，缺少时联网安装。包暂未进行发布者代码签名，附有 SHA-256 校验文件。源码分支与本地安装包的验证状态见 [0.2.2 验证记录](docs/VALIDATION_0.2.2.md)。
