...
在 [Releases](../../releases) 下载与你的平台对应的包并完整解压。预览无需安装Python或连接机械手。首次启动不自动连接、不启动电机。

| 平台 | 启动入口 | 实机连接 |
|---|---|---|
| Windows x64 | `WujiStudio/WujiStudio.exe` | 配置Linux SDK控制端 |
| Ubuntu 22.04+ x64 | `WujiStudio/WujiStudio` | Linux SDK控制端，可用本机SSH |
| macOS 14+ Apple Silicon | `WujiStudio.app` | 配置Linux SDK控制端 |
...
下载包包含独立运行环境；关闭网页后本机服务保留供再次打开。端口默认8781。若已有旧工作台运行，请先关闭旧服务，或用环境变量 `WUJI_STUDIO_PORT` 指定另一端口。停止网页采集不等于硬件急停。
...
在目标平台安装 `requirements-build.txt` 后运行 `python build.py`。各平台在原生系统构建并自检。仓库提供`ci/build.yml.example`自动构建模板；当前发布账户未授权workflow权限，因此本次采用Windows、Ubuntu、Mac分别构建。需要启用Actions时由仓库维护者将模板放入`.github/workflows/build.yml`。不是在Windows上把同一个可执行文件改名给三个平台。
...
本项目部分基础路径代码、测试和文档由Claude辅助，主维护者审核；控制逻辑与发布内容由主维护者核验。详见 [第三方来源与许可证](THIRD_PARTY_NOTICES.md)。
...
Download the matching Windows x64, Ubuntu x64 or macOS Apple Silicon package under Releases. Extract the whole archive. The Python runtime is bundled. Packages are unsigned; macOS builds are not notarized. The UI opens as a Chrome/Edge/Chromium app window when available, otherwise in your default browser. Linux needs a graphical session and OpenGL libraries.

**Hardware requires a configured Linux SDK controller via SSH on all desktop platforms**; see [controller setup](controller/README.md). This does not promise native Windows/macOS SDK support. UI/render tests do not constitute hardware acceptance. Existing gains and motion/fault handling are not changed by the visual redesign.
