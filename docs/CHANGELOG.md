...
- 编辑区分为动作列表与播放设置，固定底部操作区，长列表不会挤走启动和停止入口。
- 每条明确显示速度、重复次数与顺序控制；重复支持 1–20 次，沿用原后台范围。
- 「补齐全部动作」不再覆盖已有节目单；旧条目、顺序、速度及循环设置继续读取。
- 持续循环单独开关，随机种子仅在随机模式高级设置内出现；输入无效时不启动。
- 先选屏幕预览或真实手，再显示一个对应启动按钮；未连接直接说明原因。多手展示采用同一选择方式。
- 启动后回到模型画面，播放期间用节目单状态面板替代单动作控制，避免重复暂停入口。
- 桌面维护接口可只读打开节目单供布局检查；此操作不能发起播放或电机动作。
- 保留原生磨砂和已有有限回弹，未改 SDK、标定、轨迹、控制频率或设备参数。


## 0.2.15 — 柔和回弹与阻尼过渡

- 按钮轻按后自然回弹；弹窗轻移缩放、分区短淡入，折叠箭头和设置开关局部过渡。
- 设置新增可直接试用的回弹预览，遵循应用及系统“减少动态效果”，切换后立即停止预览。
- 离线生成曲线，浏览器执行有限动画，无新增空闲帧循环、桌面捕获、滚动接管或模糊动画。
- 停止入口不缩放；没有改变任何机械手控制、标定或求解器参数。
- 保留同一个软件与原有用户设置。Windows 安装版验证见 [验收记录](VALIDATION_0.2.15.md)。


## 0.2.14 — 清晰的操作入口与标定说明

- 右上角连接浮层使用机械手/手套状态卡与图标导航行，替换散落的蓝色链接；连接、标定、映射分别直达正确分区。
- 展示、校准、映射和设置统一主次按钮、文档入口、折叠行、焦点和禁用样式；停止动作恢复明确的按钮外形。
- 保留宫格、原生磨砂和已有动作逻辑，不重新引入桌面采样或背景折射。
- 标定页说明用户/左右侧档案的持久化与跨控制环境迁移；映射页说明参数改变机械手目标，不改写手套原始测量。
- 官方六姿势采集/求解继续调用 CLI。原版 Studio 插图、人体模型和触觉 Visualization 不宣称已经嵌入，详见 [标定与映射说明](CALIBRATION_AND_TUNING.md)。


## 0.2.13 — 连接布局与关节定位

- 机械手、手套连接集中显示，校准宫格与准备操作并排；配置和预设折叠。
- 20 格输出调整联动关节图解，支持一代/二代左右模型共 80 个位置参考。
- 模型索引、原生关节名和已核对设备节点分开显示；未核对不推定为真实对应。
- 改进中英文、深色选择对比与小窗口布局；不改变电机、求解器或标定协议。
- Opus 5.5 max 辅助基础组件，主助手复核。见 [验证记录](VALIDATION_0.2.13.md)。

## 0.2.12 — 六宫格校准与软件内开源映射

- 六个官方姿势用三列两行格子引导；官方确认采集才勾选，详细配置收起。
- 官方参数矩阵接入固定开源求解器，提供环境准备、应用、SDK 回退及真实新帧确认。
- 独立依赖和进程保留 SDK 环境；按关节名核对一代/二代左右模型，二代运行使用与画面一致的 Beta 2。
- 修复首次依赖加载超时、Hand 1 模型舍入与优化器热启动限位不一致。
- 修复 Windows 安装版控制程序路径多退一层的问题；空目录或缺失控制程序不再被当作部署成功，环境检测保留实际错误原因。
- 保留实际反馈/目标区分与单独跟随入口；本次没有启用真实机械手。
- 本地 Claude Opus 5.5 max 完成基础宫格组件，主助手审核。详细边界见 [验证记录](VALIDATION_0.2.12.md) 与 [使用说明](CALIBRATION_AND_TUNING.md)。
