...
DEFAULT=dict(host='',port=22,username='',key_filename='',known_hosts='',agent_directory='',python='python3')
...
def validate(values):
    ...
    for k in ('agent_directory','python'):
...
def load_config():
    ...
    return validate(json.loads(p.read_text(encoding='utf-8'))) if p.exists() else dict(DEFAULT)
...
def bridge_client():
    import paramiko
    c=load_config()
    if not all(c[k] for k in ('host','username','agent_directory','python')):
