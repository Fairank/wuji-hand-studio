(()=>{
 ...
 let state=null,pending=false,lease=null,leaseAt=0,seen=false,optionsKey='',lastMessage='';
 ...
 'glove-platform-text':['Ubuntu 本机可完成接收、映射和控制，无需虚拟机。Windows / Mac 工作台可连接同网段的 Linux 控制电脑或小主机；只有一台 Windows / Mac 时，可用 Linux 虚拟机，但网络发现需要正确桥接。本页使用官方 Linux 映射库，不要求 ROS 2。','Ubuntu can receive, map and control locally, with no VM. Windows/Mac can use a Linux controller PC or mini PC on the device network. A Linux VM is another option and needs suitable network bridging. This page uses the official Linux retarget library and requires no ROS 2.'],
 ...
 function render(){
  ...
  const key=JSON.stringify([g.devices,g.users,g.user?.user_id]);
  if(key!==optionsKey){optionsKey=key;const old=$('glove-device').value;$('glove-device').replaceChildren(new Option(t('请选择手套','Select a glove'),''));for(const d of g.devices||[])$('glove-device').add(new Option(`${d.serial} · ${d.address}`,d.serial));if([...$('glove-device').options].some(o=>o.value===old))$('glove-device').value=old;else if(g.devices?.length===1)$('glove-device').value=g.devices[0].serial;
   $('glove-user').replaceChildren();for(const u of g.users||[])$('glove-user').add(new Option(u.display_name+(u.is_default?' · Default':''),u.user_id));$('glove-user').value=g.user?.user_id||'';}
  ...
  for(const id of ['glove-device','glove-user','glove-timeout'])$(id).disabled=pending||receiving||connecting;
  $('glove-disconnect').disabled=pending||!g.busy;
  $('glove-prepare').disabled=pending||!stream.fresh||state?.device_profile?.generation!=='hand2'||!!feedback.device_id||hw.active!==undefined;
  ...
  $('glove-follow-note').textContent=state?.device_profile?.generation==='hand1'?t('本版支持一代手的官方手套映射预览；一代实机跟随尚未接入。','This version previews official Hand 1 glove mapping. Hand 1 hardware follow is not integrated.'):t('先连接机械手反馈，检查手套与设备的左右手是否一致，再由你开启跟随。','Prepare hand feedback and verify matching hand sides before starting follow.');
  if(lease){if(hw.active)seen=true;else if(hw.active===false&&(seen||Date.now()-leaseAt>5000))lease=null;}
  const tbody=$('glove-joint-rows');if(tbody.parentElement.parentElement.parentElement.open){tbody.replaceChildren();const rows=new Map((feedback.latest?.joints||[]).map(j=>[j.nid,j.position_rad]));for(let i=0;i<20;i++){const tr=document.createElement('tr');for(const text of [String(i+1),f(stream.fresh?stream.q?.[i]:null,3),f(hw.active?hw.applied_target_rad?.[i]:null,3),f(!feedback.stale?rows.get(Math.floor(i/4)*5+i%4+1):null,3)]){const td=document.createElement('td');td.textContent=text;tr.append(td);}tbody.append(tr);}}
 }
 ...
 async function act(c){pending=true;lastMessage='';render();try{const d=await post(c);if(d.lease){lease=d.lease;leaseAt=Date.now();seen=false;}if(c.name==='glove_stop'||c.name==='glove_disconnect')lease=null;}catch(e){lastMessage=e.message;}finally{pending=false;render();}}
 ...
 $('glove-prepare').onclick=()=>act({name:'glove_prepare',address:$('glove-hand-address').value.trim()});
