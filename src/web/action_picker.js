...
(() => {
  ...
  window.WujiActionPicker={create({root,text,onSelect}){
    ...
    root.innerHTML=`<label for="studio-action" id="studio-action-label"></label><div class="action-select-row"><select id="studio-action"></select><button type="button" id="choose-letter"></button><button type="button" id="choose-digit"></button></div><p class="action-selection-note" id="action-selection-note"></p><div id="phrase-strip" class="phrase-strip"></div><p id="performance-status" class="performance-status" role="status"></p><a id="performance-export" class="performance-export" hidden></a>`;
    ...
    function dialogLabels(){
      ...
      $('symbol-help').textContent=text(kind==='letters'?'按文字顺序播放，空格处停顿，重复字母重新起势。J / Z 为固定手腕的动态近似；其他字母也不是标准手语。选择后点击播放。':'选择后仍需点击播放，不会自动启动机械手。',kind==='letters'?'Plays in order with word pauses and repeated-letter separation. J/Z use finger-only motion approximations. These are not certified signs. Press play separately.':'Selection does not start the hand. Press play separately.');
    ...
    function render(){
      ...
      $('choose-letter').textContent=text('输入文字…','Text…');$('choose-digit').textContent=text('数字…','Digit…');
    ...
    render();
    return {restore(id,value){if(catalog.some(x=>x.id===id)){selected=id;if(typeof value==="string"&&value)phrase=value;render();}},setCatalog(data){catalog=data;render();},setPreview(value){allowPreview=value;if(!value&&catalog.find(x=>x.id===selected)?.hardware===false){selected=lastHardware;onSelect(selected);}render();},render,setBusy(value){busy=value;[select,$('choose-letter'),$('choose-digit')].forEach(x=>x.disabled=busy||!catalog.length);if(dialog.open)draft();},update(state){const p=state.playback,h=state.hardware;const active=p?.active&&p.action===selected;const idx=h?.active&&h.action===selected&&h.text===phrase?h.token_index:active&&p.text===phrase?p.token_index:-1;[...$('phrase-strip').children].forEach((e,i)=>e.setAttribute('aria-current',String(i===idx)));$('performance-status').textContent=h?.active?h.trial_phase||'':active?p.label+' · '+text('预览','Preview')+' '+Math.round(100*((!p.running&&p.cycles&&p.elapsed_s>=p.duration_s*p.cycles)?1:(p.elapsed_s%p.duration_s)/p.duration_s))+'%':'';},get payload(){return selected==='text_sequence'?{text:phrase}:{};},get selected(){return selected;}};
  }};
})();
