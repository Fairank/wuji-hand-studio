(function(){
 ...
 window.addEventListener('console-state',e=>{const s=e.detail;select.disabled=busy||s.glove?.busy||s.connection!=='disconnected'||s.hardware.active!==false;if(s.device_profile?.id&&s.device_profile.id!==selected){selected=s.device_profile.id;render();}const first=selected.startsWith('hand1'),unsupported=first&&!['open','fist'].includes($('studio-action')?.value);if(unsupported){if($('trial-start'))$('trial-start').disabled=true;const n=document.querySelector('.action-selection-note');if(n)n.textContent=text('一代候选动作仅供画面预览，请选择画面预览播放','Hand 1 candidate: use Preview; hardware action is unavailable');}const h=$('pose-console')?.querySelector('h2');if(h&&info){const p=info.profiles.find(p=>p.id===selected);h.textContent='MuJoCo · '+text(p.zh,p.en);}});
