(function(){
 ...
 function render(){
  ...
  for(const node of [$('studio-action'),$('trial-action'),$('play-action')])if(node){const option=node.querySelector('option[value="official_opposition"]');if(option)option.textContent=first?text('对指候选 · 仅预览','Opposition candidate · preview only'):text('官方二代'+(selected.endsWith('left')?'左':'右')+'手对指录制','Official Hand 2 '+(selected.endsWith('left')?'left':'right')+' recording');}
 }
