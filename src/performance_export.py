...
def export_program(action,text,profile_id,format='json'):
    ...
    if p['generation']=='hand1':
        ...
        result['points']=[dict(t=min(k/100,data['duration_s']),q=adapted(sample(action,min(k/100,data['duration_s']),text,loop=False)['q']),interpolation='linear') for k in range(math.ceil(data['duration_s']*100)+1)]
    ...
    count=math.ceil(data['duration_s']*1000)
